...
function TImgSlide(ele, direction, picwidth, picheight, speed, ease){
    ...
    self.picwidth = picwidth || parseFloat($.getStyle(ele).width);          //图片宽度
	self.picheight = picheight || parseFloat($.getStyle(ele).height);       //图片高度
...
TImgSlide.prototype = {
    bindMouse: function(){
        ...
        for (var i = 0; i < self.length; i++){
			$.on(self.slideNums[i], "mouseover", (function(index){
                return function(){
			...
			$.on(self.slideNums[i], "mouseout", function(){ self.autoRun();});
		}
    },
    autoRun: function(){
        var self=this;
