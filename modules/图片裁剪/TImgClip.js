...
TImgClip.prototype = {
	init: function(){ //初始化函数
		...
		$.on(self.PCanvas, 'mousedown', function(e){ self.onDown(e, context); });
		$.on(document, 'mouseup', function(){ self.isClip = false; self.isDrag = false; });
		$.on(self.PCanvas, 'mousemove', function(e){ self.onMove(e, context); });
		$.on(self.PCanvas, 'mouseout', function(){ self.onOut(context); });	
		//将预览canvas元素设置为禁止选中
