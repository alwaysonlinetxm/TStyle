...
TPages.prototype = {
	...
	setPageBg: function(){ //设置选中页项背景
		...
		for (var j = 2; j < a.length-2; j++){
			if (j-1 == self.current) {
				$.addClass(a[j], self.currentStyle);
			} else {
				$.removeClass(a[j], self.currentStyle);
			}
		}
	}
}
