...
TPlayer.prototype = {
	init: function(){ //初始化函数
		var self = this;
		//注册鼠标事件
		$.on(self.playButton, 'mouseup', function(){ self.playSong(); });
		$.on(self.stopButton, 'mouseup', function(){ self.stop(); });
		$.on(self.previousButton, 'mouseup', function(){ self.previous(); });
		$.on(self.nextButton, 'mouseup', function(){ self.next(); });
		$.on(self.bar, 'mousedown', function(e){ self.barTo(e); });	
		$.on(self.barEnd, 'mousedown', function(){ self.barEndDrag = true; });
		$.on(document, 'mouseup', function(){ self.barEndDrag = self.volumeEndDrag = false; });
		$.on(self.bar, 'mousemove', function(e){ if(self.barEndDrag){ self.barTo(e); };});
		$.on(self.volumeIcon, 'mouseup', function(){ self.mute(); });
		$.on(self.volume, 'mousedown', function(e){ self.volumeTo(e); });
		$.on(self.volumeEnd, 'mousedown', function(){ self.volumeEndDrag = true; });
		$.on(self.volume, 'mousemove', function(e){ if(self.volumeEndDrag){ self.volumeTo(e); };});
		//将预览拖动快元素设置为禁止选中
	...
	setCurrentInfo: function(num, flow, width, rate){ //设置当前曲目信息，参数是曲目编号，是否流动，流动速率，流动框长度
		...
		self.infoTimer = setInterval(function(){
			...
			//整条信息已不在显示范围内
			if (-left >= $.getStyle(self.currentInfo).width.slice(0, -2)){
	...
	mute: function(){ //静音
		...
		if (self.media.muted){
			//切换音量图标
			$.removeClass(self.volumeIcon, self.muteClass);
			$.addClass(self.volumeIcon, self.volumeClass);
			self.media.muted = false;
		} else {
			$.removeClass(self.volumeIcon, self.volumeClass);
			$.addClass(self.volumeIcon, self.muteClass);
	...
	play: function(){ //播放
		var self = this;
		//切换按钮图标
		$.removeClass(self.playButton, self.pauseClass);
		$.addClass(self.playButton, self.playClass);
		self.media.play();
		//设置待播放曲目初始信息
	...
	pause: function(){ //暂停
		var self = this;
		//切换按钮图标
		$.removeClass(self.playButton, self.playClass);
		$.addClass(self.playButton, self.pauseClass);
		self.media.pause();
	},
	stop: function(){ //停止
		var self = this;
